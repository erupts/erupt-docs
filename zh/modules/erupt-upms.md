...
默认开启**严格角色菜单策略**（[`erupt.upms.strict-role-menu-legal`](/zh/guide/configuration)）：非超管用户在此分配菜单时，候选菜单树仅包含自己已拥有的菜单（取其全部启用角色的并集），无法给角色授予本就没有的权限；超管不受限制。需要注意角色上原有的、超出操作人可见范围的菜单不会显示在树中，该用户保存后会被移除，此类角色请由超管维护。
...
详细使用方法请参考 [Open API 开放接口 →](/zh/advanced/open-api)
