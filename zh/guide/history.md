...
本页收录 Erupt 1.x 各版本的更新日志与历史文档入口。当前 2.x 版本的更新内容请查看 [更新日志](/zh/guide/changelog)。

:::tip 版本说明
1.x 与 2.x 为两条独立的版本线：1.14.x 是 1.x 的最后一个系列，2.0.0 起升级至 Hibernate 6 并引入不兼容变更，升级前请先阅读 [升级指南](/zh/guide/upgrade)。
:::
...
🌟 [erupt-cube](/zh/modules/pro/erupt-cube) 支持子模型、高级分组筛选、相对日期过滤，新增富文本报表、树图、热力图，KPI 及图表支持时间对比分析
...
🌟 [默认语言](/zh/guide/configuration)切换为 en-US，默认菜单数据使用英文存储
...
🌟 增加 [EruptTag](/zh/advanced/extend) 注解，修饰于 erupt 同级的注解，修饰后的注解值会返回给前端

🌟 新增 [DataProxy 全局拦截器](/zh/advanced/post-data-proxy)，全局拦截任何 erupt 类的行为 [#3364ab](https://github.com/erupts/erupt/commit/3516a0f2b3e6b4437cd2d84e4f4489a3ff3364ab)

🌟 dataProxy 增加 [validate](/zh/advanced/data-proxy#dtOCn) 方法用于自定义检验 erupt 数据是否符合规则 [#1f0a64](https://github.com/erupts/erupt/commit/54a2142b55f82ea0c97735c1b515d6d2a71f0a64)

🌟 ShowBy 注解更名为 [Dynamic](/zh/annotation/dynamic)，且支持动态的必填、只读、显示等动态能力

🌟 EruptUser 增加负责组织字段，用于配置组织负责人的身份信息

🌟 支持 OAuth2 授权，可轻松对接飞书、Google等平台的用户体系 [单点登录 （OAuth2）](/zh/advanced/auth)
...
> 1.12.x 升级 1.13.x 请务必参考：[/guide/upgrade](/zh/guide/upgrade)
>
...
>
> 1.12.x 升级 1.13.x 详见：[/guide/upgrade](/zh/guide/upgrade)
>
...
🧩 如果对象继承自 [MetaModel](/zh/advanced/utils) 时，eruptDao.persist 与 eruptDao.merge可实现人和时间字段自动填充

🧩 Excel 模板导出时 Choice 组件下拉项支持全量渲染（解决下拉列表最多255的限制）

🌟 [新增德语、葡萄牙语、印尼语、阿拉伯语支持](/zh/advanced/i18n#TTkZY)

🌟 数据库注释信息支持[多语言生成](/zh/advanced/i18n#MiihI)（新增字段与表时时生效）

🌟 新增 [Markdown](/zh/field-types/markdown) 组件的支持，感谢 [ssdlm_aifast](https://gitee.com/ssdlm_aifast) 贡献的代码[#12](https://gitee.com/erupt/erupt-web/pulls/12)
...
🧩 Layout 注解增加 [tableWidth](/zh/annotation/layout#grAm1) 配置，用于手动定义表格的总宽度  
🧩 Choice 组件增强联动能力易用性，[Choice 组件联动机制从前端联动改为后端联动](/zh/field-types/choice#S1jRs)
...
🌟 自定义按钮支持折叠配置 [fold](/zh/annotation/row-operation)，可将自定义按钮折叠展示, 适用于按钮过多的场景

🌟 erupt 支持增、删、改事件传递，监听erupt系统类与第三方类的变化，用于解耦 [🪡 监听Erupt类变化](/zh/advanced/event-listener)

🌟 Excel 导入能力支持按需导入，自由定义导入时插入或更新等操作 [⚔️ 自定义数据逻辑（ DataProxy ）](/zh/advanced/data-proxy#dtOCn)

🌟 @erupt 注解支持[自定义UI注解](/zh/advanced/extend)，可以将其他类注解的信息传递给前端，用于自定义解析，扩展注解边界

🌟 开源 [erupt-websocket](/zh/modules/erupt-websocket) 模块，可在任何地方执行前端JS脚本，用于错误提示，异步结果通知等频繁交互场景

🌟 erupt-tenant 兼容 erupt-report 可开发租户报表
...
🧩 自定义按钮渲染默认是增加：[MULTI_ONLY](/zh/annotation/row-operation#UApIg)，该模式多行按钮不会显示在单行中

🧩 新增 [DataProxyContext](/zh/advanced/data-proxy#VHBn6) 工具类，支持在DataProxy中通过上下文的方式获取当前类对象与参数等数据

🌟 表格支持按[单元格设置颜色](/zh/advanced/data-proxy#EINcZ)

🌟 表格支持按行数据动态调整编辑与删除能力 [⚔️ 自定义数据逻辑（ DataProxy ）](/zh/advanced/data-proxy#BPV07)

🌟 新增多租户插件，适合企业开发SaaS应用 [多租户 erupt-tenant](/zh/modules/pro/erupt-tenant/)
...
🌟 erupt-report 表格支持显示百分比进度条

🌟 erupt-report 表格支持配置长文本类型、链接类型
...
🌟 增加全局函数 [msg/modal/notify](/zh/advanced/frontend-notify)

🌟 tpl 支持使用[抽屉方式](/zh/annotation/row-operation#UApIg)打开
...
🌟 erupt 注解支持[热构建](/zh/advanced/misc)，修改注解值无需重启服务

🌟 choice 组件支持 [trigger](/zh/field-types/choice#HNqh7) 配置，选择组件值可以填充到其他组件

🌟 重构 [erupt-job](/zh/modules/erupt-job) 模块，支持多机集群部署
...
🌟 自定义按钮支持从已选数据中[初始化 eruptClass 表单值](/zh/annotation/row-operation#zlxgJ)

🌟 Layout 注解增加 [refreshTime](/zh/annotation/layout#grAm1) 配置，用于控制页面接口数据自动更新控制
...
🌟 [新增颜色选择组件](/zh/field-types/color)

🌟 提升数据渲染性能，表格数据超过 100 会自动开启虚拟滚动

🌟 自定义数据源支持[全局 power 控制](/zh/advanced/custom-datasource)，数据源可以全局的控制页面能力而不是通过 @Power 注解二次控制
...
**🐞** erupt-report 解决存在必填项时自定义模板404的 bug

🧩 erupt-report 兼容 JDK17

🧩 筛选场景下支持清除 choice 组件已输入值

🌟 无障碍能力支持灰色模式

🌟 erupt-report 查询维度支持**参照表格**组件，表格内部支持虚拟滚动，列筛选，列排序等能力

🌟 自定义按钮增加调用时提示文本配置能力（[callHint](/zh/annotation/row-operation#UApIg)），为空则表示不提示

🌟 增加 [Linq.J](https://github.com/erupts/Linq.J) 依赖，基于内存的对象查询语言，用于内存级对象查询与 Lambda 映射

🌟 [userinfo](/zh/advanced/rest-api) 接口增加返回组织编码、岗位编码、角色编码
...
🌟 erupt-report 定义图表数据为空时的占位展示 UI

🌟 erupt-report 图表渲染自动写入ID方便自定义样式或者动态 JS 处理

🌟 erupt-report 优化词云图交互样式
...
🌟 erupt-report：图表数据支持 excel 下载

🌟 erupt-report：图表支持备注信息展示

🌟 erupt-report：图表支持列信息自适应，会根据类型自动定义列名
...
🧩 优化checkbox组件样式限制最大高度，[checkbox组件支持渲染remark](/zh/field-types/checkbox)
...
🌟 [增加分页能力支持](/zh/annotation/layout)：支持分页大小，分页数配置

🌟 [支持分页方式配置](/zh/annotation/layout#kgWTy)：前端分页、后端分页、不分页

破坏性更新：app.js中window.eruptSiteConfig.login与window.eruptSiteConfig.logout事件移动至window.eruptEvent中，详见：[⚙️ 参数 & 样式配置](/zh/guide/configuration)
...
🌟 choice 组件支持[联动](/zh/field-types/choice#S1jRs)
...
🌟 增加评分组件：[RATE](/zh/field-types/rate)

🌟 增加水印配置：[erupt-app.water-mark](/zh/guide/configuration)，默认开启

🌟 增加 [@Layout](/zh/annotation/layout) 布局注解，可配置列固定、表单单列布局

🌟 view注解支持打开tpl自定义模板

🌟 全新的权限控制注解，请勿使用@EruptRouter，详见：[#nitieg](/zh/annotation/power)

🌟 完全重构多语言能力，增加法语、俄语、韩语支持，全框架无死角支持国际化

🌟 erupt-report增加词云 + 桑基图 + 弦图等图表类型支持

## 1.12.x
**1.11.x → 1.12.x升级指南详见↓**

[v 1.12.X](/zh/guide/upgrade)

## 1.11.7(2023-05-28) <Badge type="tip" text="Spring Boot 2.7.10" />
🌟 增加工作流模块 [erupt-flow](/zh/modules/pro/erupt-flow/) 感谢：[@hlhutu](https://github.com/hlhutu) 贡献完整的流程引擎代码
...
🌟 新增基于 @OneToOne 注解的一对一组件支持（[COMBINE](/zh/field-types/combine)）

🌟 erupt cloud 增加 [validate-access-token](/zh/modules/erupt-cloud#ta62E) 配置，用于设置 node 节点注册时是否校验 Access Token
...
🌟 [富文本编辑器](/zh/field-types/html-editor#UApIg)组件支持自定义文件上传路径配置 PR：[#135](https://github.com/erupts/erupt/pull/135)
...
🌟 **@view **注解提供 [**ifRender**](/zh/annotation/dynamic) 配置，支持动态表格列渲染

🌟 **@edit** 注解提供 [**ifRender**](/zh/annotation/dynamic) 配置，支持动态表单渲染
...
🌟 [DataProxy](/zh/advanced/data-proxy) 新增searchCondition方法，支持过滤组件自定义查询条件配置

🌟 [AutoComplete](/zh/field-types/auto-complete)组件支持数据联动，可获取其他表单对象的值，应对更多业务场景
...
🌟 自定义模板支持获取当前菜单能力（获取当前页面菜单数据: [🧲 工具类（util）](/zh/advanced/utils)）
...
🔥 开源 [erupt-cloud](/zh/modules/erupt-cloud) 分布式开发erupt node节点，构建通用云配置中心
...
🐞 修复 erupt-report 有查询项不显示图表的 BUG
...
🌟 erupt-report 数据源管理支持驱动自动获取

🌟 erupt-report 支持图表缓存与报表缓存功能

🌟 增加 [MetaModel](/zh/advanced/utils#18420919e0323a3fe4c3eecde0518e10) 工具类，可不关联用户表的情况下记录当前操作用户

🌟 新增 EruptModule 类，用于管理与实现扩展模块

🌟 增加字段覆盖功能，子类可覆盖父类的字段，提高复用性，字段用[@EruptSmartSkipSerialize](/zh/advanced/extend)修饰
...
+ 🌟 增加前端多语言支持
+ 🌟 增加服务端异常信息多语言支持
+ 🌟 [多数据源](/zh/advanced/datasource)支持 Hikari 连接池配置
+ 🌟 BoolType默认值设置为 Y / N
