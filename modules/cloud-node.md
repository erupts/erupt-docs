...
erupt:
  cloud-node:
    ...
    # 可选：当前 node 节点地址，如果网络环境复杂无法准确发现 node 节点则需要配置此项
    host-address: ['xxx']
...
<img src="/cloud/node-add.png" width="1429">

获取 access-token（如果看不到该按钮则说明未分配令牌查看权限）：

<img src="/cloud/node-token.png" width="1436">
...
<img src="/cloud/node-start.png" width="983">

启动成功后，server 端会显示当前 node 节点实例数、erupt 类数量、版本等信息：

<img src="/cloud/node-registered.png" width="1413">
...
> 获取上下文、自动写入当前操作用户等操作详见进阶文档。

## 在 Server 端关联 Node 实体

1. 前往系统管理 → 菜单维护 → 新增
2. 类型值填写：`节点名.类名`，例如节点名为 `NodeTest`，则填 `NodeTest.NodeDemo`

<img src="/cloud/menu-config.png" width="1723">

刷新页面即可看到在 node 节点配置的页面：

<img src="/cloud/node-page.png" width="1920">
...
| erupt-tookit | 工具类，例如 choice 组件的 SQL 查询能力 |
