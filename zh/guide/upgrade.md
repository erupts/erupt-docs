...
2.0.0 对 erupt-monitor 进行了**完全重写**，新增了 JVM 诊断、GC 状态、HikariCP API 连接池监控等能力。由于菜单结构与旧版完全不同，旧菜单数据无法自动替换，**必须手动清理后重启**才能正确生成新菜单。

### 操作步骤

1. **删除 `.erupt` 目录**（位于项目工作目录，即 JVM 启动路径下）：

   ```bash
   rm -rf .erupt
   ```

   > `.erupt` 目录保存了模块初始化标记文件，删除后下次启动会重新执行所有模块的菜单初始化逻辑。

2. **手动删除"系统监控"下的所有菜单**：登录后台 → 系统设置 → 菜单管理，找到"系统监控"（或 Monitor）根菜单，将其下所有子菜单及根菜单本身全部删除。

3. **重启应用**，系统自动生成新版 Monitor 菜单。

:::warning
若跳过以上步骤直接升级，系统监控页面将出现路由不匹配或空白页问题。
:::
