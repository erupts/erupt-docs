# Erupt Report 低代码数据可视化

erupt-report 是 erupt 体系内的通用报表图表模块，零前端代码，仅需配置即可完成数据分析能力。支持纯 SQL 定义报表图表，**零前后端代码**实现复杂数据分析，支持十几种查询维度、十几种图表组件与动态函数，适合有 SQL 编写能力但需要可视化功能的团队。

:::warning 🎉 开源预告
erupt-report 将于 **2026 年 8 月 23 日** 正式开源，敬请期待。
:::
...
**Q1 · 和开源版 erupt 有什么差异？**
开源 erupt 提供 `@Erupt` + `@EruptField` 基础注解能力（CRUD、表单、列表）。erupt-report 是商业模块，额外提供报表图表的可视化配置层。该模块将于 **2026 年 8 月 23 日** 正式开源。

**Q2 · 怎么交付？**
付款后，提供你的 GitHub 用户名，作者会把你加入私有仓库 `erupts/erupt-report` 的 collaborator，可直接 clone 源码、本地编译、按需引用，并发一份接入文档。
...
- **无商用限制**：可用于商业产品（含二次销售你的成品，但不可直接卖 erupt-report 源码本身）
