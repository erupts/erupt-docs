...
| # | 能力 | 是怎么挂上来的 |
| --- | --- | --- |
| 1 | **UI 渲染**（Table / Form） | [`@EruptField`](/annotation/erupt-field) · 30 种 [`EditType`](/field-types/) |
| 2 | **RBAC 权限** | [`@Power`](/annotation/power) 8 个开关 + `PowerHandler` 动态权限 |
| 3 | **REST API**（自动） | `EruptDataController` 在 `/erupt-api/data/*` 自动暴露 |
| 4 | **Auto DDL** | Hibernate `ddl-auto: update`，注解改 → 表结构改 |
| 5 | **i18n**（12 种语言） | [`I18nTranslate.$translate(key)`](/advanced/i18n) + CSV 词典 |
| 6 | **DataProxy 业务钩子** | [`beforeAdd / afterAdd / ...` 10+ 钩子](/advanced/data-proxy) |
| 7 | **Lambda 查询** | [`EruptLambdaQuery<T>`](/advanced/erupt-dao) 类型安全 DSL |
| 8 | **AI Agent** | [`erupt-ai-claw`](/modules/erupt-ai-claw) 引入即用，LLM 零代码 CRUD |
| 9 | **审批流** | [`@EruptFlow`](/modules/pro/erupt-flow) 实体即流程载体 |
| 10 | **跨服务聚合** | [`erupt-cloud-node-jpa`](/modules/cloud-node) 心跳上报中央 admin |
...
**1. UI 渲染**——列表页、新建 / 编辑表单、Excel 导入导出、搜索、排序、批量操作、关联懒加载，全部由 [`@EruptField`](/annotation/erupt-field) 驱动。[`EditType`](/field-types/) 提供 30 种字段类型，从最朴素的 [`INPUT`](/field-types/input) / [`NUMBER`](/field-types/number) / [`DATE`](/field-types/date) 到 [`HTML_EDITOR`](/field-types/html-editor) / [`CODE_EDITOR`](/field-types/code-editor) / [`MARKDOWN`](/field-types/markdown) / [`MAP`（地图）](/field-types/map) / [`SIGNATURE`（手写签名）](/field-types/signature) / [`REFERENCE_TREE`](/field-types/reference-tree) / [`REFERENCE_TABLE`](/field-types/reference-table)——后台需要的字段输入器全在这了。

**2. RBAC 权限**——[`@Power`](/annotation/power) 8 个独立开关：`add / edit / delete / query / viewDetails / export / importable / print`。可以让客服只能 query、财务只能 export、管理员才有 delete。8 个静态开关不够用？挂一个 `PowerHandler`，里面就是普通 Spring Bean，`@Autowired` 拿 `RoleService` / `TenantContext` 都行——**动态权限不是 SaaS 后台拖一下，而是 6 行 Java**。
...
**4. Auto DDL**——`application.yml` 设 `hibernate.ddl-auto: update`，加字段、改类型、加索引——重启即同步表结构。**开发期一份 SQL 脚本都不用写**，直接改 Java 注解。详见 [数据源支持](/guide/database)。
...
> 详见文档：[数据代理 DataProxy](/advanced/data-proxy)
...
[`@Erupt(dataProxy = CustomerProxy.class)`](/annotation/erupt) 引用即生效。10+ 钩子覆盖增删改查全生命周期 + Excel 导入导出 + 字段校验。
...
> 详见文档：[数据库操作 EruptDao](/advanced/erupt-dao)
...
## 五、杀手锏一：[`erupt-ai-claw`](/modules/erupt-ai-claw) —— LLM 零代码 CRUD 全实体
...
底层 [`erupt-ai`](/modules/erupt-ai) 模块自带 17 个 LLM provider——ChatGPT / Claude / DeepSeek / Gemini / Grok / Qwen / Moonshot / Ollama / OpenAI 兼容协议……admin 里一键切换，**今天 DeepSeek 跑 demo、明天 Ollama 跑生产，业务代码不动**。

这一层的工程性质是——**AI 不是另一个独立服务、不是聊天框、不是浮层。它是 `@Erupt` 元数据图直接喂给 LLM**。元数据 = UI = LLM Tool，三者同源，所以 LLM 接管 admin 不需要额外代码。

## 六、杀手锏二：[`@EruptFlow`](/modules/pro/erupt-flow) —— 实体即审批流载体
...
## 七、杀手锏三：[`erupt-cloud`](/modules/erupt-cloud) —— 微服务 JPA 远程聚合
...
每个微服务引入 [`erupt-cloud-node-jpa`](/modules/cloud-node)，**保留自己独立的 JPA 实体和数据库**，但在启动时把 `@Erupt` 注解的元数据（菜单、字段、权限）通过 HTTP 心跳上报到 [cloud-server](/modules/cloud-server)。
...
## 八、兜底：[多数据源](/advanced/datasource) + JdbcTemplate
...
四种姿势都是 Spring 生态最原生的写法——**用了 JPA 不代表锁死了**。详见 [数据库操作 EruptDao](/advanced/erupt-dao) 和 [多数据源](/advanced/datasource)。
...
> 完整依赖配置与项目初始化见 [快速部署](/guide/quick-start)。

1. 实体在 `@Entity` 旁加 [`@Erupt`](/annotation/erupt)，启动 → admin 已经在
